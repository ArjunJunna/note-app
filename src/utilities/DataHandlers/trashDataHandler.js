...
const addNoteToTrashHandler = async (
  note,
  token,
  userDataDispatch,
  setFetchingTrash
  ...
  try {
    ...
    if (response.status === 201) {
      ...
      toast.success('Your note is added to Trash...');
...
const restoreNoteFromTrashDataHandler = async (
  note,
  token,
  userDataDispatch,
  setFetchingTrash
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      userDataDispatch({ type: 'SET_NOTES', payload: response?.data?.notes });
      setFetchingTrash(false);
      toast.success('Your note is restored...');
    }
  } catch (error) {
    toast.error(error.response.data.errors[0]);
  }
...
const deleteNoteDataHandler = async (
  note,
  token,
  userDataDispatch,
  setFetchingTrash
